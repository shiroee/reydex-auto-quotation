...
/**
 * Search box for the service-reports index.
 *
 ...
 * the server re-queries. Same shape as `CertificatesSearch`.
 */
export function ServiceReportsSearch({ term }: { term: string }) {
  ...
  function search(next: string) {
    ...
    startTransition(() => {
      router.push(
        query
          ? `/service-reports?${SEARCH_PARAM}=${encodeURIComponent(query)}`
          : "/service-reports",
      );
    });
  }
  ...
  return (
    ...
    >
      <input
        ...
        placeholder="Search ref. no., customer, address, project or system"
