...
/**
 * Search box for the items index.
 *
 * Same shape as the other two: a real `<form method="get">`, so it still narrows
 * the list if the JS bundle never arrives, and once it has, `onSubmit` navigates
 * client-side while `useTransition` reports the pending state.
 */
export function ItemsSearch({ term }: { term: string }) {
  ...
  function search(next: string) {
    ...
    startTransition(() => {
      router.push(
        query ? `/items?${SEARCH_PARAM}=${encodeURIComponent(query)}` : "/items",
      );
    });
  }
